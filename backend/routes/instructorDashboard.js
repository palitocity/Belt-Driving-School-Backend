...
router.get('/profile/:id', authenticateToken, async (req, res) => {
    try {
        ...
        const instructor = await User.findById(instructorId)
            ...
            .populate({
                path: 'instructorDetails.assignedStudents',
                select: 'fullName email phone studentDetails.progress studentDetails.courseLevel address isVerified instructorDetails.licenseNumber',
            });
...
router.put('/profile/:id', authenticateToken, async (req, res) => {
    try {
        const instructorId = req.params.id;
        const { fullName, phone, specialization, experienceYears, address, licenseNumber } = req.body;

        // Find the instructor and update their information
        const instructor = await User.findById(instructorId);
        if (!instructor || instructor.role !== 'instructor') {
            return res.status(404).json({ error: 'Instructor not found.' });
        }
        ...
        instructor.licenseNumber = licenseNumber || instructor.licenseNumber;
...
router.post('/assign-student', authenticateToken, async (req, res) => {
    try {
        ...
        const student = await User.find({ email, role: 'user' });
        if (!student) {
